...
library MarketAdmin {
  ...
  function setMarketOverrides(
    ...
    require(
      marketsByBaseAssetSymbol[marketOverrides.baseAssetSymbol].exists,
      'Market does not exist'
    );

    marketOverrides.isActive = marketsByBaseAssetSymbol[
      marketOverrides.baseAssetSymbol
    ].isActive;
    marketOverrides.lastOraclePriceTimestampInMs = 0;
    marketOverrides.oraclePriceInPipsAtDeactivation = 0;

    marketOverridesByBaseAssetSymbolAndWallet[marketOverrides.baseAssetSymbol][
      wallet
    ] = marketOverrides;
  }
}
