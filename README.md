...
- Due to transaction gas limits, it is possible for an inactive wallet with open positions to fall outside of the lazy application window. As such, Exchange includes `updateWalletFundingForMarket` for the off-chain systems to proactively trigger lazy updates if necessary.
